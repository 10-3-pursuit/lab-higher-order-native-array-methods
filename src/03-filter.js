...
function getSongsBySaib(songs) {
  const saibArr = songs.filter(song => song.artist === "Saib")
  return saibArr
}
...
function getSongsOverThreeMinutes(songs) {
  const threeMinArr = songs.filter(song => song.runtimeInSeconds > 180)
  return threeMinArr
}
...
function getTitleTracks(songs) {
  const sameTitles = songs.filter(song => song.title === song.album)
  return sameTitles
}
