...
function getSongsOverThreeMinutes(songs) {
  const songsOverThreeMinutes = songs.filter((song) => song.runtimeInSeconds > 180);
...
function getTitleTracks(songs) {
  const sameTitleAsAlbum = songs.filter((song) => song.title === song.album);
  return sameTitleAsAlbum; 
}
