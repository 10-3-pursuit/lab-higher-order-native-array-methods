...
function getSongsBySaib(songs) {
  const artistSongs = songs.filter((song)=> song.artist === "Saib");
  return artistSongs;
}
...
function getSongsOverThreeMinutes(songs) {
  const artistSongs = songs.filter((song)=> song.runtimeInSeconds > 180);
  return artistSongs;
}
...
function getTitleTracks(songs) {
  const artistSongs = songs.filter((song)=> song.title === song.album);
  return artistSongs;
}
