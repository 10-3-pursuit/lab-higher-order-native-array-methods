...
function getSongDetails(songs) {
  const songDetails = songs.map((song) => {
    const copiedItem = {...song};
    return `${copiedItem.title} by ${copiedItem.artist}`
  });
...
//So i did some research and this is a way to put them into an object, i had a hard time figuring that out.
