...
function getSongTitles(songs) {
   songs.map(song => songs.splice(songs.indexOf(song),1,song.title))
   return songs
}
...
function getSongDetails(songs) {
  songs.map(song => songs.splice(songs.indexOf(song),1,`${song.title} by ${song.artist}`))
  return songs
}
...
function getTitleAndArtist(songs) {
  songs.map(song => {
    ...
    songs.splice(songs.indexOf(song),1,obj)
     })
  return songs
}
