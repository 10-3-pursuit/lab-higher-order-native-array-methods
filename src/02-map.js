...
function getSongTitles(songs) {
  ...
  const allSongsTitles = songs.map((song) => song.title)
  console.log(allSongsTitles);

  return allSongsTitles;
}
...
function getSongDetails(songs) {
  const allSongsTitlesWithArtist = songs.map((song) => `${song.title} by ${song.artist}`)
  return allSongsTitlesWithArtist;
};
...
function getTitleAndArtist(songs) {
   const songsandTitles = songs.map((song) =>  ({
    [song.title]: song.artist
   }));
   return songsandTitles;
}
