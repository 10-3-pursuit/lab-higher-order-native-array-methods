...
function getSongTitles(songs) {
  const allSongs = songs.map((song) => song.title) 
  return allSongs
}
...
function getSongDetails(songs) {
  const allArtistAndSongs = songs.map((song) => `${song.title} by ${song.artist}`)
  return allArtistAndSongs
}
...
function getTitleAndArtist(songs) {
  const titleAndArtistKey = songs.map((song) => ({
    [song.title]: song.artist
  }))
  return titleAndArtistKey
}
