...
function getSongTitles(songs) {
  const songsTitleArray = songs.map((song)=> song.title)
  return songsTitleArray;
}
...
function getSongDetails(songs) {
  const songsTitleArray = songs.map((song)=> `${song.title} by ${song.artist}`)
  return songsTitleArray;
}
...
function getTitleAndArtist(songs) {
  const songsTitleArray = songs.map((song)=> {
