...
function getSongTitles(songs) {
  const titleArr = songs.map(song => song.title)
  return titleArr
}
...
function getSongDetails(songs) {
  const songAndTitle = songs.map(song => `${song.title} by ${song.artist}`)
  return songAndTitle
}
...
function getTitleAndArtist(songs) {
  const songAndArtist = songs.map(song => ({[song.title]: song.artist}))
