...
function allSongsAreOverTwoMinutes(songs) {
  const resultArr = songs.every((song) => song.runtimeInSeconds > 120)
  return resultArr
}
...
function anySongIsOverFourMinutes(songs) {
  const resultArr = songs.some((song) => song.runtimeInSeconds > 240)
  return resultArr
}
...
function anySongIsByPeanut(songs) {
  const resultArr = songs.some((song) => song.artist === 'Peanut')
  return resultArr
}
