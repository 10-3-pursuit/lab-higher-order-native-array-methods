...
// SYNTAX: 
// const somePass = arrayName.some(function(item) {
  // Return true or false based on some condition involving item
// });
...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every(song => song.runtimeInSeconds > 180 || false)
}
...
function anySongIsOverFourMinutes(songs) {
  return songs.some(song => song.runtimeInSeconds > 240 || false)
}
...
function anySongIsByPeanut(songs) {
  return songs.some(song => song.artist === 'Peanut' || false)
}
