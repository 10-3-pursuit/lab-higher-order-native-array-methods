...
function allSongsAreOverTwoMinutes(songs) {
  const overTwoTrue = songs.every((song) => song.runtimeInSeconds > 120)
  return overTwoTrue
}
...
function anySongIsOverFourMinutes(songs) {
  const someOverFour = songs.some((song) => song.runtimeInSeconds > 240)
  return someOverFour
}
...
function anySongIsByPeanut(songs) {
  const somePeanut = songs.some((song) => song.artist === 'Peanut')
  return somePeanut
}
