...
function printAllSongTitles(songs) {
  const songTitles = songs.forEach((song) => console.log(song.title));
  return songTitles;
}
...
function printSongDetails(songs) {
  const songTitleAndArtist = songs.forEach((song) =>
...
function printSongTitlesOverThreeMinutes(songs) {
  const titlesOverThreeMinutes = songs.forEach((song) => {
    if (song.runtimeInSeconds > 180) {
      console.log(song.title);
    }
  });
  return titlesOverThreeMinutes;
}
