...
function findPinkElephantsByTimestreet(songs) {
  const pinkElephantsSong = songs.find(song => song.title === "Pink Elephants");
...
function findFirstSongUnderThreeMinutes(songs) {
  const lessThan3minsSong = songs.find(song => song.runtimeInSeconds < 180);
  return lessThan3minsSong; 
}
...
function findFirstTitleTrack(songs) {
  const songTrack = songs.find(song => song.title === song.album);
  return songTrack;
}
