...
function findPinkElephantsByTimestreet(songs) {
  const foundSong = songs.find((song)=>song.title === "Pink Elephants");
  return foundSong;
}
...
function findFirstSongUnderThreeMinutes(songs) {
  const foundSong = songs.find((song)=>song.runtimeInSeconds < 180);
  return foundSong;
}
...
function findFirstTitleTrack(songs) {
  const foundSong = songs.find((song)=>song.title === song.album);
  return foundSong;
}
