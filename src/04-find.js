...
function findPinkElephantsByTimestreet(songs) {
  const resultArr = songs.find((song) => song.title === 'Pink Elephants')
  return resultArr
}
...
function findFirstSongUnderThreeMinutes(songs) {
  const resultArr = songs.find((song) => song.runtimeInSeconds < 180)
  return resultArr
}
...
function findFirstTitleTrack(songs) {
  const resultArr = songs.find((song) => song.title === song.album)
  return resultArr
}
