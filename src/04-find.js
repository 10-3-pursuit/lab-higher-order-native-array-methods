...
function findPinkElephantsByTimestreet(songs) {
  const wheresPink = songs.find((song) => song.title === 'Pink Elephants')
  return wheresPink
}
...
function findFirstSongUnderThreeMinutes(songs) {
  const underThree = songs.find((song) => song.runtimeInSeconds < 180)
  return underThree
}
...
function findFirstTitleTrack(songs) {
  const sameTitle = songs.find((song) => song.title === song.album)
  return sameTitle
}
