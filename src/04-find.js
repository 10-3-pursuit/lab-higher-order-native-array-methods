...
function findPinkElephantsByTimestreet(songs) {
  const foundPinkElephants = songs.find((song) => {
    if (song.title === "Pink Elephants") {
      return song;
    }
  });
  return foundPinkElephants;
}
...
function findFirstSongUnderThreeMinutes(songs) {
  const underThreeMinutes = songs.find((song) => {
    if (song.runtimeInSeconds < 180) {
      return song;
    }
  });
  return underThreeMinutes;
}
...
function findFirstTitleTrack(songs) {
  const songTitleEqualsAlbumTitle = songs.find((song) => {
